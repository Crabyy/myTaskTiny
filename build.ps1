...
$stage = Join-Path ([IO.Path]::GetTempPath()) ('myTinyTask-build-' + [Guid]::NewGuid().ToString('N'))
New-Item -ItemType Directory -Path $stage | Out-Null
try {
    $metadata = @"
using System.Reflection;
[assembly: AssemblyTitle("myTinyTask")]
[assembly: AssemblyProduct("myTinyTask")]
...
"@
    ...
    $exe = Join-Path $stage 'myTinyTask.exe'
    ...
    Copy-Item -LiteralPath $exe -Destination (Join-Path $release 'myTinyTask.exe') -Force
    ...
    Set-Content -LiteralPath (Join-Path $release 'SHA256SUMS.txt') -Value "$hash  myTinyTask.exe" -Encoding ASCII
    $archive = Join-Path $root "releases\myTinyTask-$version.zip"
    Compress-Archive -Path (Join-Path $release '*') -DestinationPath $archive -Force
    }
    if (!$NoDeploy) {
        $installed = Join-Path $root 'myTinyTask.exe'
        $backup = $null
        if (Test-Path -LiteralPath $installed) {
            $active = Get-Process -ErrorAction SilentlyContinue | Where-Object { $_.Path -eq $installed }
            if ($active) { throw 'Save your recording and close myTinyTask before rebuilding.' }
    ...
    Write-Host "Built myTinyTask v$version."
