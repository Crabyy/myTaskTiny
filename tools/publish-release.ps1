...
Push-Location $root
$notesFile = Join-Path ([IO.Path]::GetTempPath()) ('myTinyTask-notes-' + [Guid]::NewGuid().ToString('N') + '.md')
try {
    ...
    $notes = $entry.Groups[1].Value.Trim() + "`n`nDownload myTinyTask.exe for the portable Windows app, or the ZIP for the app plus source and documentation. Save your work and close myTinyTask before replacing its EXE. Keep your .mtt recordings and settings files."
    ...
    $exe = Join-Path $root "releases\$version\myTinyTask.exe"
    ...
    $assets = @($exe,(Join-Path $root "releases\myTinyTask-$version.zip"),(Join-Path $root "releases\$version\SHA256SUMS.txt"))
    & gh release create $tag @assets --repo Crabyy/myTinyTask --verify-tag --draft --title "myTinyTask $version" --notes-file $notesFile
    if ($LASTEXITCODE -ne 0) { throw 'Release creation failed. Check GitHub before retrying.' }
    & gh release edit $tag --repo Crabyy/myTinyTask --draft=false --latest
